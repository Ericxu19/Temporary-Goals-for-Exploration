...
#from envs.robosuite.sawyer import SawyerReach, GoalBasedSawyerLift
...
from envs.spritelu.spriteworld.configs.protoge.c_maze import make_c_maze 
...
def make_env(args):
  ...
  elif 'umaze' in args.env.lower():
    env_fn = lambda: PointMaze2D()
    eval_env_fn = lambda: PointMaze2D(test=True)
  elif 'sawyerreach' in args.env.lower():
    env_fn = lambda: SawyerReach()
    eval_env_fn = lambda: SawyerReach()
  elif 'sawyerlift' in args.env.lower():
    use_dense = True
    objgrip=False
    if 'sparse' in args.env.lower():
      use_dense = False
    if 'objgrip' in args.env.lower():
      objgrip = True
    env_fn = lambda: GoalBasedSawyerLift(use_dense=use_dense, objgrip=objgrip)
    eval_env_fn = lambda: GoalBasedSawyerLift(test=True, objgrip=objgrip, use_dense=use_dense)
  ...
  elif 'c_maze' in args.env.lower():
    env_fn = lambda: make_c_maze()
    eval_env_fn = lambda: make_c_maze()
  elif 'sprite' in args.env.lower():
    env_fn = lambda: make_sprite_placement_env()
    eval_env_fn = lambda: make_sprite_placement_env()
  elif 'spritestatic' in args.env.lower():
    env_fn = lambda: make_sprite_placement_env()
    eval_env_fn = lambda: make_sprite_placement_env()
