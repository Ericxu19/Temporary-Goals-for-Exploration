...
class GoalSuccessPredictor(mrl.Module):
  ...
  def _setup(self):
    ...
    self.optimizer = torch.optim.Adam(self.goal_discriminator.model.parameters(), lr =0.0005 )
  ...
  def validate(self):
    ...
    if len(self.replay_buffer_validation.buffer.trajectories) > self.batch_size :
      trajs = self.replay_buffer_validation.buffer.sample_trajectories(self.batch_size, group_by_buffer=True, from_m_most_recent=self.history_length)
      successes = np.array([np.any(np.isclose(traj, 0.), axis=0) for traj in trajs[2]])
      ...
      if hasattr(self, 'logloss'):
        ...
        f = open('successvalidate.csv', 'a+')
        writer = csv.writer(f)
        row = [loss]
        writer.writerow(row)
        f.close()
        fn = open('successvalidatenum.csv', 'a+')
        writer = csv.writer(fn)
        row = [numcorrect/total]
        writer.writerow(row)
        fn.close()
