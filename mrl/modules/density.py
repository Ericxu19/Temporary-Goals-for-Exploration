...
class IndependentKernelDensity(mrl.Module):
  """
  IndependentKernelDensity
  """
  def __init__(self, item, optimize_every=10, samples=10000, kernel='gaussian', bandwidth=0.1, normalize=True, 
    log_entropy=False, tag='', buffer_name='replay_buffer'):
    ...
    self.bandwidth = bandwidth
    self.normalize = normalize
    self.ikde_sample_mean = [0.]
    self.ikde_sample_std = [1.]
    self.ready = False
    self.log_entropy = log_entropy
    self.buffer_name = buffer_name
  ...
  def _optimize(self, force=False):
    ...
    if force or (self.step % self.optimize_every == 0 and len(buffer)):
      self.ready = True
      sample_idxs = np.random.randint(len(buffer), size=self.samples)
      ikde_samples = buffer.get_batch(sample_idxs)
      idims = ikde_samples.shape[1]//self.goaldim

      if self.ikde == []: 
        for i in range(idims):
          self.ikde.append(KernelDensity(kernel=self.kernel, bandwidth=self.bandwidth))
        self.ikde_sample_mean = [0.] * idims
        self.ikde_sample_std = [1.] * idims


      entropies = []
      for i in range(0, ikde_samples.shape[1], self.goaldim):
        j = i // self.goaldim
        samples = ikde_samples[:, i:i+self.goaldim]
        if self.normalize:
            self.ikde_sample_mean[j] = np.mean(samples, axis=0, keepdims=True)
            self.ikde_sample_std[j]  = np.std(samples, axis=0, keepdims=True) + 1e-4
            samples = (samples - self.ikde_sample_mean[j]) / self.ikde_sample_std[j]

        self.ikde[j].fit(samples)

        # Now also log the entropy
        if self.log_entropy and hasattr(self, 'logger') and self.step % 250 == 0:
          # Scoring samples is a bit expensive, so just use 500 points
          num_samples = 500
          s = self.ikde[j].sample(num_samples)
          entropies.append(-self.ikde[j].score(s)/num_samples + np.log(self.ikde_sample_std[j]).sum())

      if entropies:
        for j, entropy in enumerate(entropies):
          self.logger.add_scalar('Explore/{}_entropy_{}'.format(self.module_name, j), entropy, log_every=500)
        self.logger.add_scalar('Explore/{}_entropy_total'.format(self.module_name), sum(entropies), log_every=500)

  def evaluate_log_density(self, samples):
    assert self.ready, "ENSURE READY BEFORE EVALUATING LOG DENSITY"
    res = np.zeros((samples.shape[0],))
    for i in range(0, samples.shape[1], self.goaldim):
      res += self.ikde[i//self.goaldim].score_samples((samples[:, i:i+self.goaldim] - self.ikde_sample_mean[i//self.goaldim]) / self.ikde_sample_std[i//self.goaldim])
  ...
  def save(self, save_folder):
    self._save_props(['ikde', 'ikde_sample_mean', 'ikde_sample_std', 'ready'], save_folder)

  def load(self, save_folder):
    self._load_props(['ikde', 'ikde_sample_mean', 'ikde_sample_std', 'ready'], save_folder)


class OldIndependentKernelDensity(mrl.Module):
  """
  Only works with 2d goals for now. Add a dimension para
  """
  def __init__(self, item, optimize_every=10, samples=10000, kernel='gaussian', bandwidth=0.1, normalize=True, 
    log_entropy=False, tag='', buffer_name='replay_buffer'):
    ...
    self.step = 0
    self.item = item
    self.ikde = KernelDensity(kernel=kernel, bandwidth=bandwidth)
    self.optimize_every = optimize_every
    self.samples = samples
    self.kernel = kernel
    self.bandwidth = bandwidth
  ...
  def _optimize(self, force=False):
    ...
    if force or (self.step % self.optimize_every == 0 and len(buffer)):
      ...
      self.ikde_sample_mean = [0.] * (ikde_samples.shape[1]//2)
      self.ikde_sample_std = [1.] * (ikde_samples.shape[1]//2)
      if self.normalize:
        i = 0
        while i+2 <= ikde_samples.shape[1]:
          self.ikde_sample_mean[i//2] = np.mean(ikde_samples[:,i: i+2], axis=0, keepdims=True)
          self.ikde_sample_std[i//2]  = np.std(ikde_samples[:,i: i+2], axis=0, keepdims=True) + 1e-4
          ikde_samples[:,i: i+2 ] = (ikde_samples[:,i: i+2 ] - self.ikde_sample_mean[i//2]) / self.ikde_sample_std[i//2]
          i += 2
        

      self.fitted_ikde = [None] * (ikde_samples.shape[1]//2)
      i = 0
      while i+2 <= ikde_samples.shape[1]:
        self.fitted_ikde[i//2] = self.ikde.fit(ikde_samples[:,i: i+2])
        i += 2
      ...
      if self.log_entropy and hasattr(self, 'logger') and self.step % 250 == 0:
        ...
        num_samples = 1000
        # TODO
        #s = self.fitted_ikde.sample(num_samples)
        #entropy = -self.fitted_ikde.score(s)/num_samples + np.log(self.ikde_sample_std).sum()
        #self.logger.add_scalar('Explore/{}_entropy'.format(self.module_name), entropy, log_every=500)

  def evaluate_log_density(self, samples):
    assert self.ready, "ENSURE READY BEFORE EVALUATING LOG DENSITY"
    re = [0]*samples.shape[0]
    i = 0
    
    while i + 2 <= samples.shape[1]:
      re += self.fitted_ikde[i//2].score_samples((samples[:,i: i+2]  - self.ikde_sample_mean[i//2]) / self.ikde_sample_std[i//2] )
      i += 2
    return re
  ...
  def evaluate_elementwise_entropy(self, samples, beta=0.):
    ...
    assert self.ready, "ENSURE READY BEFORE EVALUATING ELEMENT-WISE ENTROPY"
    #TODO
    # Not changed since not used
    log_px = self.fitted_ikde.score_samples( (samples  - self.ikde_sample_mean) / self.ikde_sample_std )
    px = np.exp(log_px)
    elem_entropy = entr(px + beta)
    return elem_entropy

  def save(self, save_folder):
    self._save_props(['ikde', 'ikde_sample_mean', 'ikde_sample_std', 'fitted_ikde', 'ready'], save_folder)

  def load(self, save_folder):
    self._load_props(['ikde', 'ikde_sample_mean', 'ikde_sample_std', 'fitted_ikde', 'ready'], save_folder)
