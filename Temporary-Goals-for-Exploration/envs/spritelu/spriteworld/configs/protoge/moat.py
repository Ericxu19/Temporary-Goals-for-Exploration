...
def make_moat_env(config=None, seed=None, slow_factor=0.2):
  ...
  gym_env.compute_reward = lambda ag, g, info: _moat_reward(ag, g, info)
  
  return gym_env


def _moat_reward(ag, g, info):
 
  reward = -(np.linalg.norm(ag - g, axis=-1) > TERMINATE_DISTANCE).astype(np.float32)
