...
def generate_static_agent_sprites(factor_dist):
  """creats a sprite in the middle
  """
  def _generate():
    dic = factor_dist.sample()
    dic['x'] = 0.45 + float(np.random.rand(1,1)/10)
    dic['y'] = 0.45 + float(np.random.rand(1,1)/10)
    return [sprite.Sprite(**dic)]
    
  return _generate

def generate_c_agent_sprites(factor_dist):
  """creats a sprite in the middle
  """
  def _generate():
    dic = factor_dist.sample()
    dic['x'] = 0.0 + float(np.random.rand(1,1)/10)
    dic['y'] = 0.0 + float(np.random.rand(1,1)/10)
    return [sprite.Sprite(**dic)]
    
  return _generate

def generate_static_sprites(factor_dist, num_sprites=1):
  """creats a sprite in the middle
  """
  def _generate():
    ...
    for i in range(n):
      dic = factor_dist.sample()
      dic['x'] = i*0.1 #+ float(np.random.rand(1,1)/20)+0.1
      dic['y'] = i*0.1 #+ float(np.random.rand(1,1)/20)+0.1
      dic['goal_x'] = 0.9+i*0.1 
      dic['goal_y'] =0.7+i*0.1 
      spritelst.append(sprite.Sprite(**dic))
    return spritelst
    
  return _generate

def generate_c_maze_barriers(factor_dist, num_sprites=1):
  def _generate():
    n = 1
    spritelst = []
    for i in range(n):
