...
class GoalGridWorldEnv(gym.GoalEnv):
    ...
    def __init__(self, obs_xy: bool, grid_size=16, max_step=100, grid_file=None, random_init_loc=True, \
                 agent_loc_file=None, goal_file=None, seed=1337):
        # Whether state observation is NxNxK or just (x,y) position of agent
        self.obs_xy = obs_xy
        ...
        if self.obs_xy:
            observation_space = spaces.Box(
                low=0,
                high=max(self.grid_size_0, self.grid_size_1),
                shape=(2,),
                dtype='uint8'
            )
        else:
    ...
    def _get_obs(self):
        ...
        """
        if self.obs_xy:
            desired_goal = np.array(self.goal_loc)
        else:
            desired_goal = self.one_hot(self.goal, len(self.objects))
        obs = {
            'observation': self._get_state(self.grid, self.obs_xy),
            'desired_goal': desired_goal, 
            'achieved_goal': self._get_state(self.grid, self.obs_xy)
        }
        return obs

    def _get_state(self, grid, obs_xy, use_one_hot=True):
        """
    ...
    def render(self, mode='human'):
        ...
        imgs = [self._get_state(self.grid, obs_xy=False, use_one_hot=False), self.goal]
